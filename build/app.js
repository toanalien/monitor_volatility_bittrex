...
(function(){
  var path, async, lodash, request, express, compression, ms, favicon, expressRobots, app, server_port, time_cache, limitOrderBook, yaId, exchangeMain, exchangeList, CACHE, WALL_DATA, LIST_PAIR, objToString, textError, differenceBetweenNumbers, percentageBetweenNumbers, flat_func, orderBook, orderBookPair, currencyPairList, loadDataPair, reg, htmlData, loading, main, request_n;
  path = require('path');
  async = require('async');
  ...
  expressRobots = require('express-robots');
  app = express();
  app.use(compression());
  app.use(expressRobots({
    UserAgent: '*',
    Disallow: '/'
  }));
  ...
  exchangeMain = process.env.EXCHANGE_MAIN || 'bittrex';
  exchangeList = (process.env.EXCHANGE_LIST || 'bitfinex, bittrex, btc-e, poloniex, exmo').replace(/\s+/g, '').split(',');
  CACHE = {
    date: 0,
    data: []
  };
  ...
  flat_func = function(x, y, z){
    var ref$;
    return (ref$ = (z - x) / (y - x) * 100) > 0 ? ref$ : 0;
  };
  ...
  htmlData = function(data, cbk){
    var css, exch, exchange, endTime, metrika, html;
    css = {
      green: "color: green",
      red: "color: red",
      black: "color: black"
    ...
    exch = {};
    exchange = [];
    lodash.forEach(exchangeList, function(v){
      exch[v] = "<a href='https://monitor-volatility-" + v + ".herokuapp.com' rel='nofollow'>" + lodash.upperFirst(v) + "</a>";
    });
    lodash.forIn(exch, function(v, k){
      if (k !== exchangeMain) {
        exchange.push(v);
      }
    });
