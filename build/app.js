...
(function(){
  var async, lodash, request, express, ms, app, server_port, time_cache, limitOrderBook, yaId, CACHE, WALL_DATA, LIST_PAIR, objToString, textError, differenceBetweenNumbers, percentageBetweenNumbers, orderBook, orderBookPair, currencyPairList, loadDataPair, reg, htmlData, loading, main, request_n;
  async = require('async');
  ...
  htmlData = function(data, cbk){
    ...
    html = [
      "<html><head><title>Анализ волатильности торговых пар биржи Bittrex</title><script type='text/javascript' src='/public/jquery.min.js'></script><script type='text/javascript' src='/public/jquery.tablesorter.js'></script><link rel='stylesheet' href='/public/style.css' type='text/css'><style type='text/css'>.tablesorter { font-size: 10pt !important; width: 1350px !important; }</style><script type='text/javascript'>$(document).ready(function() {$('table').tablesorter();});</script>" + metrika + "</head><body>", "<h2>Анализ волатильности торговых пар биржи " + bittrex + " (" + btcE + " | " + poloniex + ")</h2>", "<h3>Период: 24 ч. &nbsp;&nbsp; Время: " + new Date(CACHE.date).toLocaleTimeString('en-US', {
        timeZone: 'Europe/Moscow',
        hour12: false
      }) + " &nbsp;&nbsp;&nbsp; Обновление кэша через: " + endTime + " мин. </h3>", "<table class='tablesorter'><thead><tr><th>Пара</th><th>% волотильности</th><th>% change</th><th>Ask</th><th>Bid</th><th>Стенка на Ask</th><th>Стенка на Bid</th><th>Volume</th><th>Volume Fork</th></tr></thead><tbody>"
    ];
          ...
          loadDataPair(LIST_PAIR, function(err, data){
            ...
            if (err != null) {
              ...
              Object.keys(data.pair_max_pr).forEach(function(key){
                if (in$(key, LIST_PAIR)) {
                  sortable.push([key, data.pair_max_pr[key].max_pr, data.pair_max_pr[key].buy, data.pair_max_pr[key].sell, WALL_DATA[key].ask, WALL_DATA[key].bid, data.pair_max_pr[key].quoteVolume, data.pair_max_pr[key].volume, data.pair_max_pr[key].change]);
                }
              });
